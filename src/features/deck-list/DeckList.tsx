...
export const DeckList: React.FC<DeckListProps> = ({ 
  ...
  return (
    ...
    >
      ...
      <div className='bg-gradient-to-br from-blue-600 via-purple-600 to-blue-800 rounded-xl p-3 sm:p-4 mb-4 text-white'>
        <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4'>
          <div>
            <h1 className='text-2xl sm:text-3xl font-bold mb-2'>Мои колоды</h1>
          ...
          <div className='flex gap-3'>
            <motion.button
              ...
              className='bg-white/20 backdrop-blur-sm hover:bg-white/30 text-white px-4 py-3 rounded-xl transition-all duration-200 font-medium flex items-center gap-2 border border-white/30 cursor-pointer'
            ...
            <motion.button
              ...
              className='bg-white/20 backdrop-blur-sm hover:bg-white/30 text-white px-6 py-3 rounded-xl transition-all duration-200 font-medium flex items-center gap-2 border border-white/30 cursor-pointer'
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <Plus size={20} />
      ...
      {deckModel.decks.length === 0 ? (
        ...
        >
          <div className='bg-gradient-to-br from-blue-50 to-purple-50 rounded-2xl p-6 sm:p-8 max-w-lg mx-auto'>
            <BookOpen size={48} className='text-blue-400 mx-auto mb-4' />
            <h2 className='text-xl font-bold text-gray-800 mb-3'>Начните изучение</h2>
            ...
            <motion.button
              onClick={onCreateDeck}
              className='bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white px-6 py-3 rounded-lg transition-all duration-200 font-medium flex items-center gap-3 mx-auto cursor-pointer'
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <Plus size={20} />
          ...
          {deckModel.decks.map((deck, index) => {
            ...
            return (
              <motion.div
                ...
                className='bg-white rounded-xl border border-gray-100 p-4 sm:p-5 hover:shadow-xl hover:shadow-blue-100/50 transition-all duration-300 cursor-pointer hover:border-blue-200 group'
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                <div className='flex items-start justify-between mb-4'>
                  ...
                  <div className='flex items-center gap-2'>
                    {(stats.new > 0 || stats.due > 0) && (
                      <div className='bg-gradient-to-r from-orange-400 to-red-500 text-white text-xs px-3 py-2 rounded-full min-w-[28px] h-7 flex items-center justify-center font-bold'>
                        {stats.new + stats.due}
                      </div>
                    )}
                    <motion.button
